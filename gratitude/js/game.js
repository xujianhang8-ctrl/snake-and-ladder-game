// Pure game rules for Snakes & Ladders on the heart board.
...
// `answer` is the result of the heart question asked on a purple heart:
...
// questions: stopping on a purple heart asks a right-or-wrong question first,
// and only a wrong answer slides the token down.
export function createGame({ players, finishRule = FINISH_RULES.EXACT, questions = false }) {
  ...
  return {
    ...
    current: 0,
    finishRule,
    questions: Boolean(questions),
    winner: null,
    history: [],
  };
}

// True when this roll stops the current player on a purple heart and the game
// has to ask its question before the move can be completed.
...
// true, pass whether the question was answered correctly.
export function takeTurn(state, roll, answer = null) {
  ...
  if (asked && typeof answer !== 'boolean') {
    throw new TypeError('This roll stops on a purple heart: say whether the question was answered correctly');
  }
...
export function isValidState(state) {
  ...
  if (typeof state.questions !== 'boolean') return false;
